...
const nodes = [
  ...
  { label: "Mentors", icon: Users, angle: 72, color: "text-primary", bg: "bg-primary/15" },
  { label: "Partners", icon: Handshake, angle: 144, color: "text-accent", bg: "bg-accent/15" },
  { label: "Programs", icon: BookOpen, angle: 216, color: "text-primary", bg: "bg-primary/15" },
  { label: "Events", icon: Calendar, angle: 288, color: "text-accent", bg: "bg-accent/15" },
];
...
const EcosystemNetwork = () => {
  ...
  return (
    <section className="section-padding relative overflow-hidden section-light" ref={ref}>
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-border to-transparent" />
      <div className="absolute top-[30%] left-0 w-[400px] h-[400px] rounded-full bg-accent/[0.05] blur-[100px]" />
      <div className="absolute bottom-[20%] right-0 w-[350px] h-[350px] rounded-full bg-primary/[0.04] blur-[80px]" />
        ...
        <div className="grid items-center gap-16 lg:grid-cols-[1fr_1.2fr]">
            ...
            <div className="mt-10 space-y-4">
              {nodes.map((node, i) => (
                ...
                >
                  ...
                  <Navigation className="h-3 w-3 text-accent/30 ml-auto" />
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          >
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 400">
              <motion.circle cx="200" cy="200" r="170" fill="none" stroke="hsl(var(--accent))" strokeWidth="1" strokeDasharray="4 8"
                initial={{ pathLength: 0, opacity: 0 }} animate={isInView ? { pathLength: 1, opacity: 0.2 } : {}} transition={{ delay: 0.5, duration: 2 }} />
              <motion.circle cx="200" cy="200" r="130" fill="none" stroke="hsl(var(--primary))" strokeWidth="1" strokeDasharray="6 6"
                initial={{ pathLength: 0, opacity: 0 }} animate={isInView ? { pathLength: 1, opacity: 0.12 } : {}} transition={{ delay: 0.8, duration: 1.5 }} />
              {nodes.map((node, i) => {
                const radius = 160;
                const cx = 200 + radius * Math.sin((node.angle * Math.PI) / 180);
                const cy = 200 - radius * Math.cos((node.angle * Math.PI) / 180);
                return (
                  <motion.line key={node.label} x1="200" y1="200" x2={cx} y2={cy} stroke="hsl(var(--accent))" strokeWidth="2" strokeDasharray="6 4"
                    initial={{ pathLength: 0, opacity: 0 }} animate={isInView ? { pathLength: 1, opacity: 0.3 } : {}} transition={{ delay: 0.3 + i * 0.15, duration: 0.8 }} />
                );
              })}
              {nodes.map((node, i) => {
                ...
                return (
                  <motion.line key={`cross-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke="hsl(var(--primary))" strokeWidth="1" strokeDasharray="3 6"
                    initial={{ pathLength: 0, opacity: 0 }} animate={isInView ? { pathLength: 1, opacity: 0.12 } : {}} transition={{ delay: 1.2 + i * 0.1, duration: 0.8 }} />
                );
              })}
              <motion.circle cx="200" cy="200" r="40" fill="none" stroke="hsl(var(--accent))" strokeWidth="1"
            ...
            <motion.div className="absolute inset-[15%]" animate={{ rotate: -360 }} transition={{ duration: 28, repeat: Infinity, ease: "linear" }}>
              <div className="absolute bottom-0 right-1/4 h-1.5 w-1.5 rounded-full bg-primary/50" />
            </motion.div>
          </motion.div>
